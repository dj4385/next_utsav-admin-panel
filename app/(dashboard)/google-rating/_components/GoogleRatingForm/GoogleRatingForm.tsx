...
import { setAddGoogleRatingSuccess } from "@/lib/features/EventsSlice";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import ButtonComponent from "@/components/core/Button/Button";

const GoogleRatingForm = () => {
    ...
    const onSave = async () => {
        try {
            ...
            const res: any = await GoogleRatingService.addGoogleRating(req);
            if (res && (res.status == 200 || res.status == 201)) { 
                toast({
                    title: "Google Rating added successfully",
                    description: "Google Rating added successfully",
                })
                dispatch(setAddGoogleRatingSuccess(true));
                setRange('');
                setMin('');
                setMax('');
                setDescription('');
                ...
                toast({
                    title: "Error",
                    description: "Unable to add google rating",
                    variant: "destructive",
                })
            }
            setLoading(false);
...
export default GoogleRatingForm;
