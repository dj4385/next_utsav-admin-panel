...
import { setAddPhotographerSuccess } from "@/lib/features/EventsSlice";
...
const PhotographerForm = () => {
    ...
    const onSave = async () => {
        try {
            ...
            const res: any = await PhotographerService.addPhotographer(req);
            if (res && (res.status == 200 || res.status == 201)) { 
                toast({
                    title: "Photographer added successfully",
                    description: "Photographer added successfully",
                })
                dispatch(setAddPhotographerSuccess(true));
                setName('');
            } else {
                toast({
                    title: "Error",
                    description: "Unable to add photographer",
                    variant: "destructive",
                })
            }
            setLoading(false);
